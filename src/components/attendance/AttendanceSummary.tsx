...
import { calculateAttendanceTotals } from '@/utils/attendanceCalculation';
...
import { Calendar, Download, BarChart3, Calculator, Loader2, ChevronLeft, ChevronRight, Info, Search, FileSpreadsheet } from 'lucide-react';
...
export function AttendanceSummary() {
  ...
  // Calculate date range based on view type (moved up for dependency)
  ...
  // DB에서 근태 데이터 조회
  const { data: dbAttendance = [], isLoading: isAttendanceLoading } = useAttendanceRange(dateRange.start, dateRange.end);

  // DB 근태 데이터가 있으면 사용, 없으면 스토어 데이터 사용
  ...
  const employeeSummary = useMemo(() => {
    ...
    return filteredEmps.map((emp) => {
      ...
      // 야간교대 근무일수 × 60분 보정 (야교 엔진에서 고정 차감된 휴게를 합산 로직에서 복원)
      const nightShiftDays = empAttendance.filter(
        (att) => (att as any).workType === 'night' && ((att as any).rawCheckIn || att.checkIn) && ((att as any).rawCheckOut || att.checkOut)
      ).length;
      ...
      const attendanceRate = dateRange.daysInPeriod > 0
        ? Math.round(((presentDays + lateDays) / dateRange.daysInPeriod) * 100)
        : 0;
  ...
  }, [employees, attendance, selectedDepartment, selectedEmploymentType, selectedPayType, selectedJobCategory, dbEmployees, dateRange, settings]);
  ...
  const overallStats = useMemo(() => {
    ...
    const avgAttendanceRate = employeeSummary.length > 0
  ...
  const filteredEmployees = activeEmployees.filter(emp => 
    payTypeFilter === 'all' || emp.payType === payTypeFilter
  );
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedEmployeeIds(prev => {
        ...
        const otherSelected = prev.filter(id => !activeEmployees.find(e => e.id === id) || !filteredEmployees.find(e => e.id === id));
  ...
  const handleExcelDownload = async () => {
    const targetSummary = checkedSummaryIds.length > 0
    ...
    try {
      ...
      sheet.mergeCells(1, 1, 1, 24); // Merge cells for title
      titleRow.height = 24;
      ...
      const headerRow = sheet.addRow([
        '사원번호', '이름', '급여유형', '부서', '출근', '지각', '결근', '휴가',
        '체류시간', '휴게시간', '지각 절사', '근무시간 단위 절사', '조기퇴근 절사', '외출 절사',
        '인정근무', '정규근무시간', '연장근무시간', '야간근무시간',
        '휴일근로(8h이내)', '휴일근로(8h초과)',
        '야간교대근무시간',
        '1단계(정규+비야간)', '2단계(정규+야간)', '3단계(연장+야간)', '4단계(연장+비야간)',
        '출근율'
      ]);
      ...
      targetSummary.forEach((emp: any) => {
        sheet.addRow([
          ...
          emp.totalWorkTime,
          fmtMin(emp.totalRegularMinutes || 0),
          fmtMin(emp.totalOvertimeWorkMinutes || 0),
          fmtMin(emp.totalNightWorkMinutes || 0),
          fmtMin(emp.totalHoliday8hMinutes || 0),
          fmtMin(emp.totalHolidayOver8hMinutes || 0),
          fmtMin(emp.totalNightShiftWorkMinutes || 0),
  ...
  const handleDetailExcelDownload = async () => {
    const targetEmployees = checkedSummaryIds.length > 0
      ? employeeSummary.filter(emp => checkedSummaryIds.includes(emp.id))
      : employeeSummary;
    ...
    try {
      ...
      for (const emp of targetEmployees) {
        ...
        sheet.columns = [
          { width: 15 }, { width: 10 }, { width: 12 }, { width: 12 }, { width: 14 },
        ];
        ...
        // Accumulate totals
        let totalStay = 0, totalBreak = 0, totalLate = 0, totalOT = 0;
        let totalEarly = 0, totalOuting = 0, totalRecognized = 0;
        let totalRegular = 0, totalOvertime = 0, totalNight = 0, totalNightShift = 0;
        let totalTier1 = 0, totalTier2 = 0, totalTier3 = 0, totalTier4 = 0;
        let totalHoliday8h = 0, totalHolidayOver8h = 0;
        ...
        empRecords.forEach((r: any) => {
          ...
          if (ci && co) {
            ...
            const isHolidayDay = isWeeklyHoliday(r.date, settings) || (isPublicHoliday(r.date) && settings.apply_public_holiday);
            const bd = calculateSingleAttendance(ci, co, r.date, r.breakMinutes ?? 0, isNight, settings, isHolidayDay && !isNight);
          ...
          const row = sheet.addRow([
            ...
            ci ? new Date(ci).toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit', hour12: false, timeZone: 'Asia/Seoul' }) : '-',
            co ? new Date(co).toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit', hour12: false, timeZone: 'Asia/Seoul' }) : '-',
            workTimeStr,
          ]);
        ...
        if (empRecords.length > 0) {
          ...
          const summaryData = [
            ...
            ['휴게시간', `-${totalBreak}분`],
            ['[정책차감]', ''],
            ['  지각 절사', totalLate > 0 ? `-${totalLate}분` : '0분'],
            ['  근무시간 단위 절사', totalOT > 0 ? `-${totalOT}분` : '0분'],
            ['  조기퇴근 절사', totalEarly > 0 ? `-${totalEarly}분` : '0분'],
            ...
            ['  휴일근로(8h이내)', fmtMinutes(totalHoliday8h)],
            ['  휴일근로(8h초과)', fmtMinutes(totalHolidayOver8h)],
            ['  야간교대근무시간', fmtMinutes(totalNightShift)],
  ...
  const handleCalculatePayroll = async () => {
    ...
    try {
      // DB에 직원이 있으면 DB 기반 계산, 없으면 스토어 기반 계산
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-4">
        {/* 부서/고용형태 사이드바 필터 */}
        <FilterSidebar
          employees={employees.map(e => ({ id: e.id, department: e.department, employment_type: e.employmentType, pay_type: e.payType, job_category: dbEmployees.find(d => d.id === e.id)?.job_category || 'office' }))}
      ...
      <div className="rounded-lg border bg-card">
        ...
        <Table>
            ...
            <TableRow>
              <TableHead className="w-12">
                <Checkbox
                  ...
                  aria-label="전체 선택"
                  className={checkedSummaryIds.length > 0 && checkedSummaryIds.length < employeeSummary.length ? 'opacity-50' : ''}
                />
              </TableHead>
          ...
          <TableBody>
            {groupedSummary.length > 0 ? groupedSummary.map(group => {
              ...
              return (
                <React.Fragment key={`grp-${group.department}`}>
                  <DepartmentGroupHeader
                    ...
                    colSpan={COL_SPAN}
                  />
                  {!collapsedDepts.has(group.department) && group.items.map((emp) => (
              <TableRow 
                key={emp.id}
              ...
              >
                ...
                <TableCell>
                  {(() => {
                    ...
                    return (
                      <Badge variant="secondary" className={cn("text-xs font-normal", jobCat === 'production' ? 'bg-amber-50 text-amber-700 border-amber-200' : 'bg-slate-50 text-slate-700 border-slate-200')}>
                  ...
                  <div className="flex items-center justify-center gap-1">
                    ...
                    {emp.totalMinutes > 0 && (
                      <HoverCard openDelay={200} closeDelay={100}>
                        ...
                        <HoverCardContent className="w-64 text-sm" side="top" align="start" avoidCollisions={true} collisionPadding={16}>
                          <div className="space-y-2">
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">체류시간</span>
                              <span className="font-medium">{Math.floor(emp.totalStayMinutes / 60)}시간 {emp.totalStayMinutes % 60}분</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">휴게시간</span>
                              <span className="font-medium">-{emp.totalBreakMinutes}분</span>
                            </div>
                             <div className="text-muted-foreground text-xs font-medium mb-1">정책차감</div>
                              <div className="flex justify-between pl-2">
                                <span className="text-muted-foreground text-xs">지각 절사</span>
                                <span className="text-xs">{emp.totalLateTruncation > 0 ? `-${emp.totalLateTruncation}분` : '0분'}</span>
                              </div>
                              <div className="flex justify-between pl-2">
                                <span className="text-muted-foreground text-xs">근무시간 단위 절사</span>
                                <span className="text-xs">{emp.totalOvertimeTruncation > 0 ? `-${emp.totalOvertimeTruncation}분` : '0분'}</span>
                              </div>
                              <div className="flex justify-between pl-2">
                                <span className="text-muted-foreground text-xs">조기퇴근 절사</span>
                                <span className="text-xs">{emp.totalEarlyLeaveTruncation > 0 ? `-${emp.totalEarlyLeaveTruncation}분` : '0분'}</span>
                              </div>
                              <div className="flex justify-between pl-2">
                                <span className="text-muted-foreground text-xs">외출 절사</span>
                                <span className="text-xs">{emp.totalOutingTruncation > 0 ? `-${emp.totalOutingTruncation}분` : '0분'}</span>
                              </div>
                            ...
                            <div className="pl-2 space-y-1">
                              <div className="flex justify-between">
                                ...
                                <span className="text-xs">{Math.floor(emp.totalRegularMinutes / 60)}시간 {emp.totalRegularMinutes % 60}분</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-muted-foreground text-xs">연장근무시간</span>
                                <span className="text-xs">{Math.floor(emp.totalOvertimeWorkMinutes / 60)}시간 {emp.totalOvertimeWorkMinutes % 60}분</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-muted-foreground text-xs">야간근무시간</span>
                                <span className="text-xs">{Math.floor(emp.totalNightWorkMinutes / 60)}시간 {emp.totalNightWorkMinutes % 60}분</span>
                              </div>
                              {(emp.totalHoliday8hMinutes > 0 || emp.totalHolidayOver8hMinutes > 0) && (
                                <>
                                  <div className="flex justify-between">
                                    <span className="text-muted-foreground text-xs">휴일근로(8h이내)</span>
                                    <span className="text-xs">{Math.floor(emp.totalHoliday8hMinutes / 60)}시간 {emp.totalHoliday8hMinutes % 60}분</span>
                                  </div>
                                  <div className="flex justify-between">
                                    <span className="text-muted-foreground text-xs">휴일근로(8h초과)</span>
                                    <span className="text-xs">{Math.floor(emp.totalHolidayOver8hMinutes / 60)}시간 {emp.totalHolidayOver8hMinutes % 60}분</span>
                                  </div>
                                </>
                              )}
                              <div className="flex justify-between">
                                <span className="text-muted-foreground text-xs">야간교대근무시간</span>
                                <span className="text-xs">{Math.floor(emp.totalNightShiftWorkMinutes / 60)}시간 {emp.totalNightShiftWorkMinutes % 60}분</span>
                              </div>
                              {emp.totalNightShiftWorkMinutes > 0 && (
                                <div className="pl-4 space-y-0.5">
                                  {emp.totalNightShiftTier1Minutes > 0 && (
                                    <div className="flex justify-between">
                                      <span className="text-muted-foreground/70 text-[11px]">├ 1단계(정규+비야간)</span>
                                      <span className="text-[11px] text-muted-foreground">{Math.floor(emp.totalNightShiftTier1Minutes / 60)}시간 {emp.totalNightShiftTier1Minutes % 60}분</span>
                                    </div>
                                  )}
                                  {emp.totalNightShiftTier2Minutes > 0 && (
                                    <div className="flex justify-between">
                                      <span className="text-muted-foreground/70 text-[11px]">├ 2단계(정규+야간)</span>
                                      <span className="text-[11px] text-muted-foreground">{Math.floor(emp.totalNightShiftTier2Minutes / 60)}시간 {emp.totalNightShiftTier2Minutes % 60}분</span>
                                    </div>
                                  )}
                                  {emp.totalNightShiftTier3Minutes > 0 && (
                                    <div className="flex justify-between">
                                      <span className="text-muted-foreground/70 text-[11px]">├ 3단계(연장+야간)</span>
                                      <span className="text-[11px] text-muted-foreground">{Math.floor(emp.totalNightShiftTier3Minutes / 60)}시간 {emp.totalNightShiftTier3Minutes % 60}분</span>
                                    </div>
                                  )}
                                  {emp.totalNightShiftTier4Minutes > 0 && (
                                    <div className="flex justify-between">
                                      <span className="text-muted-foreground/70 text-[11px]">└ 4단계(연장+비야간)</span>
                                      <span className="text-[11px] text-muted-foreground">{Math.floor(emp.totalNightShiftTier4Minutes / 60)}시간 {emp.totalNightShiftTier4Minutes % 60}분</span>
                                    </div>
                                  )}
                                </div>
                              )}
        ...
        {checkedSummaryIds.length > 0 && (
          <div className="flex items-center justify-between px-4 py-3 bg-muted/50 border-t rounded-b-lg">
            <span className="text-sm text-muted-foreground">
              <span className="font-medium text-foreground">{checkedSummaryIds.length}명</span> / {employeeSummary.length}명 선택됨
            </span>
            <div className="flex items-center gap-2">
            ...
            <div className="flex items-center justify-between pb-2 border-b">
              <div className="flex items-center space-x-2">
                ...
                <label htmlFor="select-all" className="text-sm font-medium cursor-pointer">
                  {payTypeFilter === 'all' ? '전체 선택' : `${payTypeFilter === 'monthly' ? '월급' : payTypeFilter === 'daily' ? '일급' : '시급'} 전체 선택`} ({filteredEmployees.length}명)
                </label>
              </div>
              <Badge variant="secondary">
                {selectedEmployeeIds.length}명 선택됨
              </Badge>
            </div>
                ...
                <div key={emp.id} className="flex items-center space-x-2 p-2 rounded hover:bg-muted">
                  ...
                  <label htmlFor={`emp-${emp.id}`} className="flex-1 text-sm cursor-pointer">
                    ...
                    <Badge variant="outline" className="text-xs font-normal ml-1">
                      {emp.employmentType === 'regular' ? '정규직' : emp.employmentType === 'contract' ? '계약직' : '일용직'}
                    </Badge>
