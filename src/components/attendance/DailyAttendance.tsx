...
import { Calendar, Filter, UserCheck, UserX, AlertCircle, Palmtree, Pencil, ChevronLeft, ChevronRight, Info, Search, X, Phone, Mail, Building2, CreditCard, CalendarDays, Users } from 'lucide-react';
...
export function DailyAttendance() {
  const { employees: storeEmployees, attendance: storeAttendance, checkIn: storeCheckIn, checkOut: storeCheckOut } = useEmployeeStore();
  ...
  // DB 근태 데이터 조회
  const { attendance: dbAttendance, isLoading, checkIn: dbCheckIn, checkOut: dbCheckOut, bulkUpdateStatus, updateShiftType } = useAttendance(selectedDate);
  ...
  const filteredAttendance = useMemo(() => {
    ...
    return result;
  }, [attendance, selectedDepartment, selectedEmploymentType, selectedPayType, selectedJobCategory, employees, dbEmployees]);
  ...
  const stats = {
    ...
    unrecorded: filteredAttendance.filter((a) => a.status === 'unrecorded').length,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-4">
        {/* 부서/고용형태 사이드바 필터 */}
        <FilterSidebar
          employees={employees.map((e: any) => ({ id: e.id, department: e.department, employment_type: e.employmentType, pay_type: e.payType, job_category: dbEmployees.find(d => d.id === e.id)?.job_category || 'office' }))}
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[
          ...
          <div
            ...
            onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') setActiveStatFilter(prev => prev === item.key ? null : item.key); }}
            className={cn(
      ...
      {activeStatFilter && (() => {
        const statFilteredEmps = filteredAttendance
          .filter(a => a.status === activeStatFilter)
          .map(a => {
        ...
        const filterLabel: Record<string, string> = {
          present: '출근', late: '지각', absent: '결근', unrecorded: '미기록', leave: '휴가',
        };
        return (
          <div className="rounded-lg border bg-card animate-fade-in">
            <div className="p-4 border-b flex items-center justify-between">
              <h3 className="text-sm font-semibold">{filterLabel[activeStatFilter]} 명단 ({statFilteredEmps.length}명)</h3>
              <button onClick={() => setActiveStatFilter(null)} className="text-muted-foreground hover:text-foreground transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
            <Table>
              ...
              <TableBody>
                {statFilteredEmps.length > 0 ? statFilteredEmps.map(emp => (
      ...
      <div className="rounded-lg border bg-card">
        <Table>
          ...
          <TableBody>
            {groupedAttendance.length > 0 ? groupedAttendance.map(group => {
              const groupWorkMinutes = group.items.reduce((sum, record) => {
                ...
                if (rec.rawCheckIn && rec.rawCheckOut) {
                  ...
                  const breakdown = calculateSingleAttendance(rec.rawCheckIn, rec.rawCheckOut, rec.date || selectedDate, rec.breakMinutes || 0, isNight, settings);
              ...
              return (
                <>
                  ...
                  {!collapsedDepts.has(group.department) && group.items.map((record) => {
                    ...
                    return (
                      <TableRow key={record.id} id={`att-row-${record.employeeId}`} data-highlighted={highlightedEmployeeId === record.employeeId || undefined}>
                        <TableCell>
                          <Checkbox
                        ...
                        <TableCell>
                          <Badge variant="outline" className="font-medium">
                            {employees.find(e => e.id === record.employeeId)?.payType === 'monthly' ? '월급' : 
                             employees.find(e => e.id === record.employeeId)?.payType === 'daily' ? '일급' : '시급'}
                          </Badge>
                        </TableCell>
                        ...
                        <TableCell>
                          {rec.hasDbRecord ? (
                            <Select
                              value={rec.shiftType === 'day' || rec.shiftType === 'night' ? rec.shiftType : undefined}
                            ...
                            >
                              <SelectTrigger className="w-20 h-7 text-xs">
                                <SelectValue placeholder={rec.shiftType && rec.shiftType !== 'day' && rec.shiftType !== 'night' ? '미분류' : undefined} />
                              </SelectTrigger>
                              <SelectContent>
                            ...
                            {hoverData && (
                              <HoverCard openDelay={200} closeDelay={100}>
                                <HoverCardTrigger asChild>
                                  <button className="inline-flex items-center justify-center rounded-full text-muted-foreground hover:text-foreground transition-colors" aria-label="근무시간 상세">
                                ...
                                <HoverCardContent className="w-64 text-sm" side="left">
                                  <div className="space-y-2">
                                    <div className="flex justify-between">
                                      ...
                                      <span className="font-medium">{formatWorkHours(hoverData.stayMinutes)} <span className="text-xs text-muted-foreground">({hoverData.actualCheckIn}~{hoverData.actualCheckOut})</span></span>
                                    </div>
                                    <div className="flex justify-between">
                                    ...
                                    <div className="flex justify-between pl-2">
                                      ...
                                      <span className="text-xs">{hoverData.lateTruncation > 0 ? `-${hoverData.lateTruncation}분` : '0분'}</span>
                                    </div>
                                    <div className="flex justify-between pl-2">
                                      <span className="text-muted-foreground text-xs">근무시간 단위 절사</span>
                                      <span className="text-xs">{hoverData.overtimeTruncation > 0 ? `-${hoverData.overtimeTruncation}분` : '0분'}</span>
                                    </div>
                                    <div className="flex justify-between pl-2">
                                      <span className="text-muted-foreground text-xs">조기퇴근 절사</span>
                                      <span className="text-xs">{hoverData.earlyLeaveTruncation > 0 ? `-${hoverData.earlyLeaveTruncation}분` : '0분'}</span>
                                    </div>
                                    <div className="flex justify-between pl-2">
                                      <span className="text-muted-foreground text-xs">외출 절사</span>
                                      <span className="text-xs">{hoverData.outingTruncation > 0 ? `-${hoverData.outingTruncation}분` : '0분'}</span>
                                    </div>
                                    <div className="border-t pt-2 flex justify-between">
                                      <span className="font-semibold">인정근무</span>
                                      <span className="font-semibold text-primary">{formatWorkHours(hoverData.recognizedMinutes)}</span>
                                    </div>
                                    <div className="flex justify-between pl-2">
                                      <span className="text-muted-foreground text-xs">정규근무시간</span>
                                      <span className="text-xs font-medium">{formatWorkHours(hoverData.regularMinutes)}</span>
                                    </div>
                                    <div className="flex justify-between pl-2">
                                      <span className="text-muted-foreground text-xs">연장근무시간</span>
                                      <span className="text-xs font-medium">{formatWorkHours(hoverData.overtimeWorkMinutes)}</span>
                                    </div>
                                    <div className="flex justify-between pl-2">
                                      <span className="text-muted-foreground text-xs">야간근무시간</span>
                                      <span className="text-xs font-medium">{formatWorkHours(hoverData.nightWorkMinutes)}</span>
                                    </div>
                                    <div className="flex justify-between pl-2">
                                      <span className="text-muted-foreground text-xs">야간교대근무시간</span>
                                      <span className="text-xs font-medium">{formatWorkHours(hoverData.nightShiftWorkMinutes)}</span>
                                    </div>
                                      ...
                                      <div className="pl-4 space-y-0.5">
                                        {hoverData.nightShiftTier1Minutes > 0 && (
                                          <div className="flex justify-between">
                                            ...
                                            <span className="text-[11px] text-muted-foreground">{formatWorkHours(hoverData.nightShiftTier1Minutes)}</span>
                                          </div>
                                        )}
                                        {hoverData.nightShiftTier2Minutes > 0 && (
                                          <div className="flex justify-between">
                                            <span className="text-muted-foreground/70 text-[11px]">├ 2단계(정규+야간)</span>
                                            <span className="text-[11px] text-muted-foreground">{formatWorkHours(hoverData.nightShiftTier2Minutes)}</span>
                                          </div>
                                        )}
                                        {hoverData.nightShiftTier3Minutes > 0 && (
                                          <div className="flex justify-between">
                                            <span className="text-muted-foreground/70 text-[11px]">├ 3단계(연장+야간)</span>
                                            <span className="text-[11px] text-muted-foreground">{formatWorkHours(hoverData.nightShiftTier3Minutes)}</span>
                                          </div>
                                        )}
                                        {hoverData.nightShiftTier4Minutes > 0 && (
                                          <div className="flex justify-between">
                                            <span className="text-muted-foreground/70 text-[11px]">└ 4단계(연장+비야간)</span>
                                            <span className="text-[11px] text-muted-foreground">{formatWorkHours(hoverData.nightShiftTier4Minutes)}</span>
                                          </div>
                                        )}
                                      </div>
                                    )}
                        ...
                        <TableCell className="text-right">
                          <div className="flex justify-end gap-2">
                            ...
                            {record.checkIn && (record.status as string) !== 'unrecorded' && (
                              <Button size="sm" variant="ghost" onClick={() => setEditingRecord(record)} title="근태 수정">
        ...
        {selectedIds.length > 0 && (
          <div className="flex items-center justify-between px-4 py-3 bg-muted/50 border-t rounded-b-lg">
            <span className="text-sm text-muted-foreground">
              <span className="font-medium text-foreground">{selectedIds.length}명</span> / {filteredAttendance.length}명 선택됨
            </span>
          ...
          {selectedEmployee && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <DailyInfoItem icon={<Building2 className="w-4 h-4" />} label="부서" value={selectedEmployee.department || '-'} />
                <DailyInfoItem icon={<Users className="w-4 h-4" />} label="직위" value={selectedEmployee.position || '-'} />
                <DailyInfoItem icon={<Phone className="w-4 h-4" />} label="연락처" value={selectedEmployee.phone || '-'} />
                <DailyInfoItem icon={<Mail className="w-4 h-4" />} label="이메일" value={selectedEmployee.email || '-'} />
                <DailyInfoItem icon={<CalendarDays className="w-4 h-4" />} label="입사일" value={selectedEmployee.hire_date} />
                <DailyInfoItem icon={<CreditCard className="w-4 h-4" />} label="급여유형" value={
                  selectedEmployee.pay_type === 'monthly' ? '월급' : selectedEmployee.pay_type === 'hourly' ? '시급' : '일급'
                } />
                <DailyInfoItem icon={<CreditCard className="w-4 h-4" />} label="기본급" value={`${selectedEmployee.base_salary.toLocaleString()}원`} />
                <DailyInfoItem icon={<Building2 className="w-4 h-4" />} label="고용형태" value={
