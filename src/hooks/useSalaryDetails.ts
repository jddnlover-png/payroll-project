  ...
  const calculateAll = useMutation({
    mutationFn: async (employeeIds?: string[]) => {
      ...
      for (const emp of targetEmployees) {
        const empMonthAtt = allAttendance.filter(a =>
          a.employee_id === emp.id && a.date >= monthStart && a.date <= monthEnd
        );

        const prevCarryDays = carryMap.get(emp.id) || 0;
        const result = calculateSalaryDetail(
          emp, empMonthAtt, allAttendance, settings, yearMonth, prevCarryDays
        );
        results.push(result);
      }
